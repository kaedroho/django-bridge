...
test.describe("Navigation", () => {
  ...
  test("should show loading state during slow navigation", async ({ page }) => {
    ...
    await page
      .getByText("Navigate to a very slow view (5 second delay)")
      .click();
  ...
  test("should handle navigation to view that raises exception", async ({
    page,
  }) => {
  ...
  test("should handle navigation to view without component", async ({
    page,
  }) => {
    // Click the no component view link
    await page
      .getByText("Navigate to a view that doesn't have a component")
      .click();
